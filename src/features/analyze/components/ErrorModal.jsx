    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute inset-0 grid place-items-center p-4">
        <div
          ...
          className={`relative w-full max-w-md overflow-hidden rounded-2xl shadow-xl ring-1 ${
            isDark ? 'bg-slate-900/90 ring-slate-700 text-slate-100' : 'bg-white ring-slate-200'
          }`}
        ...
        >
          ...
          <button
            ...
            className={`absolute right-3 top-3 inline-flex h-9 w-9 items-center justify-center rounded-full transition ${
              isDark ? 'text-slate-300 hover:bg-slate-800/70 hover:text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-700'
            }`}
          >
          ...
          <div className="flex flex-col items-center px-6 pt-10 pb-6">
            <div className={`flex h-16 w-16 items-center justify-center rounded-full ${isDark ? 'bg-red-500/15' : 'bg-red-50'}`}>
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`h-9 w-9 ${isDark ? 'text-red-400' : 'text-red-600'}`}>
            ...
            <div className="mt-4 w-full px-2 text-center">
              {list.length > 0 ? (
                ...
                <p className={`text-sm ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>알 수 없는 오류가 발생했습니다.</p>
              )}
            </div>
            <div className="mt-5" />
          </div>
